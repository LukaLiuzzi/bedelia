...
const crearUsuario = async (req, res) => {
  // Definir crearUsuario antes de usarlo
  ...
  if (!correoElectronico || !nombre || !apellido || !clave) {
    ...
    bcrypt.hash(clave, 10, async (err, hashedClave) => {
      if (err) {
        ...
        res
          .status(500)
          .json({ estado: "FALLA", msj: "Error al hashear la contraseña" })
      } else {
        const usuario = {
        ...
        try {
          ...
          res
            .status(201)
            .json({ estado: "ok", msj: "Usuario creado", dato: usuarioNuevo })
        } catch (error) {
          console.error(error)
          res
            .status(500)
            .json({ estado: "FALLA", msj: "Error en la creación del usuario" })
        }
      }
    })
  }
